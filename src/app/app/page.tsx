...
const QUESTIONS = [
  {
    topic: "Datenbanken",
    q: "Welche Normalform verlangt, dass alle Nicht-Schlüsselattribute voll funktional vom gesamten Primärschlüssel abhängen?",
    opts: [
    ...
    correct: 1,
    expl: "Die 2NF verlangt volle funktionale Abhängigkeit vom gesamten Primärschlüssel — sie beseitigt partielle Abhängigkeiten.",
  },
  {
    topic: "Programmierung",
    q: "Welche Zeitkomplexität hat die binäre Suche in einem sortierten Array?",
    opts: ["O(n)", "O(1)", "O(log n)", "O(n²)"],
    correct: 2,
    expl: "Der Suchraum halbiert sich mit jedem Schritt — das ergibt logarithmische Laufzeit O(log n).",
  },
  {
    topic: "Netzwerke",
    q: "Auf welchem OSI-Layer arbeitet ein Router primär?",
    opts: [
      "Layer 2 — Sicherung",
      "Layer 3 — Vermittlung",
      "Layer 4 — Transport",
    ...
    correct: 1,
    expl: "Router treffen Weiterleitungsentscheidungen anhand von IP-Adressen — das ist die Vermittlungsschicht (Layer 3).",
  },
];

const TOPICS = [
  { name: "Datenbanken", pct: 62, score: 85 },
  { name: "Programmierung", pct: 41, score: 72 },
  { name: "Netzwerke", pct: 28, score: 64 },
  { name: "Wirtschafts- & Sozialkunde", pct: 15, score: 58 },
];
...
export default function AppPage() {
  ...
  const verdictLabel = selected === q.correct ? "Richtig!" : "Nicht ganz.";

  const topicsView = TOPICS.map((t) => ({
    ...t,
    pctLabel: t.pct + "%",
    scoreLabel: t.score + "%",
    scoreColor: passColor(t.score),
  }));

  // Confetti pieces (result screen).
  const confetti = [];
  const cols = [
    "var(--accent)",
        ...
        <main style={{ maxWidth: "1160px", margin: "0 auto", padding: "64px 28px 90px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(300px,1fr))", gap: "20px" }}>
            ...
            >
              ...
              <h3 style={{ ...heading, fontWeight: 600, fontSize: "21px", margin: "0 0 6px", letterSpacing: "-.01em", color: "var(--text)" }}>IHK Anwendungsentwicklung</h3>
              ...
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "8px" }}>
                <span style={{ fontSize: "13px", fontWeight: 600, color: "var(--muted)" }}>Fortschritt</span>
                <span style={{ fontSize: "13px", fontWeight: 700, color: "var(--accent-strong)" }}>34%</span>
              </div>
            ...
            {topicsView.map((t) => (
              ...
              >
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "14px" }}>
                  ...
                  <span style={{ fontSize: "13px", fontWeight: 700, color: "var(--muted)" }}>{t.pctLabel}</span>
                </div>
                <div style={{ height: "6px", borderRadius: "100px", background: "var(--bg-alt)", overflow: "hidden" }}>
                  <div style={{ width: t.pctLabel, height: "100%", borderRadius: "100px", background: "var(--accent)" }} />
                </div>
              </button>
            ))}
          </div>
            ...
            {answered && (
              <div style={{ animation: "eh-popIn .4s cubic-bezier(.16,1,.3,1) both", marginTop: "24px", display: "flex", gap: "13px", alignItems: "flex-start", background: "var(--bg-alt)", border: "1px solid var(--border)", borderRadius: "16px", padding: "18px 20px" }}>
                ...
                <div>
                  ...
                  <div style={{ fontSize: "15px", color: "var(--muted)", lineHeight: 1.5 }}>{q.expl}</div>
                </div>
              </div>
            )}
          ...
          <div style={{ position: "relative", zIndex: 1, textAlign: "center", animation: "eh-revUp .6s cubic-bezier(.16,1,.3,1) both" }}>
            <div style={{ position: "relative", width: "150px", height: "150px", margin: "0 auto 26px" }}>
              <svg width="150" height="150" viewBox="0 0 120 120" style={{ transform: "rotate(-90deg)" }}>
                ...
                <circle cx="60" cy="60" r="54" fill="none" stroke="var(--accent)" strokeWidth="11" strokeLinecap="round" strokeDasharray="339" strokeDashoffset="75" style={{ animation: "eh-ringFill 1.1s cubic-bezier(.16,1,.3,1) both" }} />
              </svg>
              <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
                <span style={{ ...heading, fontWeight: 700, fontSize: "42px", letterSpacing: "-.03em", lineHeight: 1 }}>78%</span>
                <span style={{ fontSize: "13px", fontWeight: 600, color: "var(--muted)" }}>Ergebnis</span>
              </div>
            </div>
            <span style={{ display: "inline-block", padding: "6px 16px", borderRadius: "100px", background: "color-mix(in oklch, var(--accent) 14%, var(--bg))", color: "var(--accent-strong)", fontSize: "14px", fontWeight: 700, marginBottom: "16px" }}>Bestanden 🎯</span>
            <h1 style={{ ...heading, fontWeight: 700, fontSize: "clamp(30px,4vw,44px)", letterSpacing: "-.03em", margin: "0 0 10px", lineHeight: 1.05 }}>Stark — du hast bestanden!</h1>
            <p style={{ fontSize: "18px", color: "var(--muted)", margin: "0 auto 40px", maxWidth: "460px" }}>39 von 50 Fragen richtig. Ein paar Themen brauchen noch etwas Feinschliff.</p>
          </div>
              ...
              {topicsView.map((t) => (
                <div key={t.name}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "8px" }}>
                    ...
                    <span style={{ fontSize: "14px", fontWeight: 700, color: t.scoreColor }}>{t.scoreLabel}</span>
                  </div>
                  <div style={{ height: "9px", borderRadius: "100px", background: "var(--bg-alt)", overflow: "hidden" }}>
                    <div style={{ width: t.scoreLabel, height: "100%", borderRadius: "100px", background: t.scoreColor }} />
                  </div>
                </div>
              ))}
            </div>
